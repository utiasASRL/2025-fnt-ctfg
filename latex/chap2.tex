...
\end{equation}
To economize notation, when labelling our factors, we will only use the $|| \mbf{x} - \mbs{\mu} ||^2_\mbs{\Sigma}$ part, corresponding to a term in the negative-log likelihood cost that we are seeking to minimize.  We may also sometimes write $p(\mbf{x}) \sim \mathcal{N}(\mbs{\mu}, \mbs{\Sigma})$ as a shorthand.
...
\begin{equation}
|| \mbf{u}_1 - \mbf{A}_1 \mbf{x} ||^2_{\mbf{Q}_1} + || \mbf{u}_2 - \mbf{A}_2 \mbf{x} ||^2_{\mbf{Q}_2}  = || \mbf{A}_1^{-1} \mbf{u}_1 - \mbf{A}_2^{-1} \mbf{u}_2 ||^2_{\mbf{Q}_{12}} + || \mbf{u} - \mbf{x} ||^2_\mbf{Q},
\end{equation}
where $\mbf{Q}_{12}$, $\mbf{Q}$, $\mbf{u}$,  are defined in Figure~\ref{fig:gausselim} and $\mbf{A}_1$ and $\mbf{A}_2$ are for now assumed invertible.  We will use this closed-form Gaussian elimination time and again in what follows.  
...
Our state-estimation problem is now to use the initial-state knowledge, $\pri{\mbf{x}}_0$, the inputs, $\mbf{v}_k$, and the measurements, $\mbf{y}_k$, to work out the likelihood of $\mbf{x}$ taking on all possible values.   This can be done exactly for this linear-Gaussian system; later for nonlinear systems we will require some approximations.  We will show that by using a combination of the Gaussian elimination and fusion operations from Section~\ref{sec:elimfus}, we can recover the classic \ac{RTS} smoother, the forward pass of which is the famous \ac{KF}.
...
\mbox{forward:} \quad\;\;\; & & \nonumber \\
\mbox{($k=1\ldots K$)} \; & & \nonumber \\
\pri{\mbf{P}}_{k,f} & = & \mbf{A}_{k,k-1} \est{\mbf{P}}_{k-1,f} \mbf{A}_{k,k-1}^T + \mbf{Q}_k, \\
\pri{\mbf{x}}_{k,f} & = & \mbf{A}_{k,k-1} \est{\mbf{x}}_{k-1,f} + \mbf{v}_k, \\
...
\end{eqnarray}
\end{subequations}
and $\mbf{K}_0 = \pri{\mbf{P}}_{0} \mbf{C}_0^T \left( \mbf{C}_0 \pri{\mbf{P}}_{0} \mbf{C}_0^T + \mbf{R}_0 \right)^{-1}$.  The forward pass is the very famous \acf{KF}, which we now understand from a factor-graph perspective.  Thus, the $\mathcal{N}\left(\est{\mbf{x}}_{k,f}, \est{\mbf{P}}_{k,f} \right)$ quantities can be referred to as {\em filtered} (accounting only for up-to-the-present measurements) while the $\mathcal{N}\left( \est{\mbf{x}}_k, \est{\mbf{P}}_k \right)$ quantities are {\em smoothed} (accounting for all measurements: past, present, and future); see Figure~\ref{fig:filtersmoother}.
