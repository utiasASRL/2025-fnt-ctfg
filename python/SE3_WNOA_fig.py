...
def plot_se3(plotter, T, scale=0.05, color=None, opacity=0.3, frame=True):
    """
    ...
    if frame:
        ...
        # Just plot a point at the origin
        plotter.add_mesh(pv.Sphere(radius=scale, center=origin), color=color, opacity=opacity)
    ...
    for k in range(K):
        ...
        for j in range(J):
            ...
            if plotting and j > 0 and j < J-1:
                ...
                plot_se3(plotter, T_tau_inv, scale=0.01, color='blue', opacity=0.05, frame=False)
