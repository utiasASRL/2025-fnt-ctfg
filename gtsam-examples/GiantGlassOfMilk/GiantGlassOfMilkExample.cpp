...
int main() {
  ...
  string config_file = "GiantGlassOfMilk/GiantGlassOfMilk.yaml";
  ...
  // Run through all states and add WNOA prior factors between neighbouring
  // states
  ...
  int skip =
      dt_meas / dt;  // Skip factor, depending on the measurement frequency
  ...
  for (unsigned int i = 0; i < times.size(); i = i + skip) {
    ...
    if (use_interpolation) {
      graph.add(PriorFactor<Point1>(X(i / skip), position_measurement,
                                    positionNoise));
      graph.add(PriorFactor<Vector1>(V(i / skip), velocity_measurement,
                                     velocityNoise));
    } else {
      graph.add(PriorFactor<Point1>(X(i), position_measurement, positionNoise));
      graph.add(
          PriorFactor<Vector1>(V(i), velocity_measurement, velocityNoise));
    }
  }
  ...
  if (use_interpolation) {
    ...
    Values results_interpolated = interpolator.interpolatePosesAndVelocities(
        graph, result, mainSolveStateSet, interpolateStateSet, covarianceMap);
